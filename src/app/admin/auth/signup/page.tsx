...
export default function SignupPage() {
    ...
    const onSubmit = async (formData: SignupFormData) => {
        try {
            setIsLoading(true);
            const { error } = await createUser(formData.name, formData.email, formData.password, 'admin', 1)
            if (error) {
                if (error.code === "auth") { //hack
                    return toast.error("That email is alreay registered.")
                } else {
            ...
            logError(error, "Client error create user")
